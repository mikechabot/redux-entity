...
module.exports = {
    ...
    makeActionCreator: function makeActionCreator(type) {
        ...
        var keys = _getArgumentKeysAtIndex(arguments, 1);
        return function () {
            return _generateAction({ type: type }, keys, arguments);
        };
    },
    /**
     ...
     */
    makeEntityActionCreator: function makeEntityActionCreator(type, entity) {
        if (!type) throw new Error('Type cannot be null/undefined');
        if (!entity) throw new Error('Entity cannot be null/undefined');
        var keys = _getArgumentKeysAtIndex(arguments, 2);
        return function () {
            return _generateAction({ type: type, entity: entity }, keys, arguments);
        };
    }
};