    ...
    describe('loadEntity()', () => {
        ...
        describe('when loadEntity() fails', () => {
            it('should dispatch FETCH_REQUEST and FETCH_FAILURE actions', (done) => {
                ...
                // Under test
                store.dispatch(loadEntity(entity, promise, false))
                    .catch(() => {
                        // Assert 2 actions were invoked
                        const actions = store.getActions();
                        expect(actions.length).toEqual(2);
        ...
        describe('when loadEntity() is configured with stage processors', () => {
            ...
            it('Stage BEFORE_FAILURE should return a new object', (done) => {
                const beforeFailure = {
                    _runBeforeFailure(dispatch, getState, error) {
                ...
                // Under test
                store
                    .dispatch(loadEntity(entity, Promise.reject(new Error('Fake error 1')), configOptions))
                    .catch(() => {
                        expect(
                            store.getActions()[1].error
                        ).toEqual(new Error('Fake error 2'))
                        done();
                    });
            });
